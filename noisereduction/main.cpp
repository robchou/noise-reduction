...
//    std::cout << "Processing " << result["input"].as<std::string>() << " -> " << result["output"].as<std::string>() << std::endl;
    std::cout << "Processing " << "input.pcm" << " -> " << "output.pcm" << std::endl;

    Log::SetLogLevel(LOG_VERBOSE);

    std::string profilePath = "./noise.pcm";
...
//    reduction.ExtractNoiseProfile(profileBuffer.get(), profileSize);
    ...
    std::ifstream src;
    std::ofstream out;
    std::string inputPath = "./input.pcm";
    ...
    std::string outputPath = "output.pcm";
